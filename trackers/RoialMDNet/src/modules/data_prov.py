...
class RegionDataset(data.Dataset):
    def __init__(self, img_dir, img_list, gt, receptive_field, pretrain_opts):
        ...
        if pretrain_opts['use_gpu']:
            self.img_crop_model.gpu_enable()
    ...
    def __next__(self):
        ...
        for i, (img_path, bbox) in enumerate(zip(self.img_list[idx], self.gt[idx])):
            ...
            padded_x1 = (neg_examples[:, 0] - neg_examples[:, 2] * (self.pretrain_opts['padding'] - 1.) / 2.).min()
            padded_y1 = (neg_examples[:, 1] - neg_examples[:, 3] * (self.pretrain_opts['padding'] - 1.) / 2.).min()
            padded_x2 = (neg_examples[:, 0] + neg_examples[:, 2] * (self.pretrain_opts['padding'] + 1.) / 2.).max()
            padded_y2 = (neg_examples[:, 1] + neg_examples[:, 3] * (self.pretrain_opts['padding'] + 1.) / 2.).max()
            ...
            crop_img_size = (padded_scene_box[2:4] * (
                        (self.pretrain_opts['img_size'], self.pretrain_opts['img_size']) / bbox[2:4])).astype('int64') * \
                            jitter_scale[0][0]
            cropped_image, cur_image_var = self.img_crop_model.crop_image(image, np.reshape(padded_scene_box, (1, 4)),
                                                                          crop_img_size)
            cropped_image = cropped_image - 128.
            if self.pretrain_opts['use_gpu']:
            ...
            jittered_obj_size = jitter_scale[0][0] * float(self.pretrain_opts['img_size'])
            ...
            pos_rois = samples2maskroi(pos_rois, self.receptive_field, (jittered_obj_size, jittered_obj_size),
                                       bbox[2:4], self.pretrain_opts['padding'])
