...
class RoIAlign(Module):
    ...
    def forward(self, features, rois):
        return RoIAlignFunction.apply(features, rois, (int(self.aligned_height), int(self.aligned_width)),
...
class RoIAlignAvg(Module):
    ...
    def forward(self, features, rois):
        pooled = RoIAlignFunction.apply(features, rois, (int(self.aligned_height) + 1, int(self.aligned_width) + 1),
                                        self.spatial_scale, self.sample_num)
...
class RoIAlignMax(Module):
    ...
    def forward(self, features, rois):
        pooled = RoIAlignFunction.apply(features, rois, (int(self.aligned_height) + 4, int(self.aligned_width) + 4),
                                        self.spatial_scale, self.sample_num)
...
class RoIAlignAdaMax(Module):
    ...
    def forward(self, features, rois):
        pooled = RoIAlignAdaFunction.apply(features, rois, (int(self.aligned_height) + 4, int(self.aligned_width) + 4),
                                           self.spatial_scale, self.sample_num)
        return max_pool2d(pooled, kernel_size=3, stride=2)
...
class RoIAlignAda(Module):
    ...
    def forward(self, features, rois):
        return RoIAlignAdaFunction.apply(features, rois, (int(self.aligned_height), int(self.aligned_width)),
                                         self.spatial_scale, self.sample_num)
