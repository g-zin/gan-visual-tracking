...
seq_home = os.environ['TRK_WD'] + '/datasets/imagenet/ILSVRC2015'
...
def index(seq_list):
    ...
    for i, seqname in enumerate(seq_list):
        ...
        seq_path = os.path.join(seq_home, 'Data/VID/train', seqname)
        gt_path = os.path.join(seq_home, 'Annotations/VID/train', seqname)
...
def main():
    ...
    val_output_path = 'pretrain/data/imagenet_vid_val.pkl'

    train_seq_list = []
    for _, cur_dir in enumerate([p for p in os.listdir(os.path.join(seq_home, 'Data/VID/train'))]):
        train_seq_list += [os.path.join(cur_dir, p) for p in
                           os.listdir(os.path.join(seq_home, 'Data/VID/train', cur_dir))]
    val_seq_list = []
    for _, cur_dir in enumerate([p for p in os.listdir(os.path.join(seq_home, 'Data/VID/train'))]):
        train_seq_list += [os.path.join(cur_dir, p) for p in
                           os.listdir(os.path.join(seq_home, 'Data/VID/train', cur_dir))]

    #data = index(train_seq_list)
    #save_db(data, output_path)

    data = index(val_seq_list)
    save_db(data, val_output_path)
