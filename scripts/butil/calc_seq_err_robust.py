...
def calc_seq_err_robust(results, rect_anno):
    ...
    if resultType == 'rect':
        ...
        for i in range(seq_length):
            # rect, c, corn = scripts.calc_rect_center(results['tmplsize'], res[i])
            rect = rect_affine_IVT(results.tmplsize, res[i])
            rectMat[i] = rect
    elif resultType == 'L1Aff' or resultType == 'affine_L1':
        for i in range(seq_length):
            # rect, c = calc_center_L1(res[i], results['tmplsize'])
            rect = rect_affine_L1(results.tmplsize, res[i])
            rectMat[i] = rect
    elif resultType == 'LK_Aff' or resultType == 'affine_LK':
        for i in range(seq_length):
            rect = rect_affine_LK(results.tmplsize,
                                                res[2 * i:2 * (i + 1)])
            rectMat[i] = rect
    elif resultType == '4corner':
        for i in range(seq_length):
            # corner = res[2*i:2*(i+1)]
            # rectMat[i] = d_to_f(m.corenr2rect(corner, nargout=1)[0])
            rect = rect_4corners(res[2 * i:2 * (i + 1)])
            rectMat[i] = rect
