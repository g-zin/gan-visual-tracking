...
class SampleGenerator:
    def __init__(self, sample_type, img_size, trans_f=1, scale_f=1, aspect_f=None, valid=False):
...
def gen_samples(generator, bb, n, overlap_range=None, scale_range=None):
    n = int(n)
    if overlap_range is None and scale_range is None:
        return generator(bb, n)
    else:
        samples = None
        remain = n
        factor = 2
        while remain > 0 and factor < 16:
            samples_ = generator(bb, remain * factor)
            idx = np.ones(len(samples_), dtype=bool)
            if overlap_range is not None:
                r = overlap_ratio(samples_, bb)
                idx *= (r >= overlap_range[0]) * (r <= overlap_range[1])
            if scale_range is not None:
                s = np.prod(samples_[:, 2:], axis=1) / np.prod(bb[2:])
