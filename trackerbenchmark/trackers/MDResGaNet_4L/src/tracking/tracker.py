...
def train(model, model_g, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4', **opts):
    ...
    for i in range(maxiter):
        ...
        if model_g is not None:
            ...
            model_g.train()
            num = res_asdn.size(0)
            mask_asdn = torch.ones(num, 2048, 3, 3)
        ...
        if model_g is not None:
            ...
            for k in range(9):
                ...
                model.eval()
                batch = batch_pos_feats.view(batch_pos, 2048, 3, 3)
                batch[:, :, col, row] = 0
            ...
            optimizer_g = set_optimizer_g(model_g, lr=opts['lr_g'], momentum=opts['momentum'], w_decay=opts['w_decay'])
...
def run_mdnet(img_list, init_bb, gt=None, **opts):
    # Init bbox
    target_bbox = np.array(init_bb)
