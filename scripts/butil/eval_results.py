from scripts.butil.calc_seq_err_robust import calc_seq_err_robust
from scripts.model.score import getScoreList, Score
from config import *
...
def calc_result(tracker, seqs, results, evalType):
    ...
    for i in range(len(results)):
        ...
        seq.errCoverage = []
        seq.errCenter = []
        seq.fps = sum(r.fps for r in subResults) / len(subResults)
        ...
        for j in range(idxNum):
            ...
            aveCoverage, aveErrCenter, errCoverage, errCenter = calc_seq_err_robust(result, anno)
    ...
    for attr in attrList:
        ...
        for seq in seqs:
            if attr.name in seq.attributes or attr.name.lower() == 'all':
                ...
                attr.fpsList.append(seq.fps)
                seqSuccessList = []
                length = len(seq.errCoverage)
                for threshold in thresholdSetOverlap:
                    seqSuccess = [score for score in seq.errCoverage if score > threshold]
        ...
        attr.fps = sum(attr.fpsList) / len(attr.fpsList)
        attr.refresh_dict()
    # end for scores
