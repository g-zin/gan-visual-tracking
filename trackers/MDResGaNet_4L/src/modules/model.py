...
def append_params(params, module, prefix):
    for child in module.children():
        for k, p in list(child._parameters.items()):
...
class MDNet(nn.Module):
    def __init__(self, model_path=None, K=1):
        ...
        self.layers = nn.Sequential(OrderedDict([
            ('conv1', nn.Sequential(res50.conv1,
                                    res50.bn1,
                                    res50.relu,
                                    res50.maxpool)),
            ('conv2', nn.Sequential(res50.layer1, res50.layer2)),
            ('conv3', nn.Sequential(
                res50.layer3,
                res50.layer4,
                nn.MaxPool2d(kernel_size=2, stride=1)
            )),
            ('fc4', nn.Sequential(nn.Linear(2048 * 3 * 3, 512),
